...
def start_test() -> Any:
    ...
    if not selected_categories:
        ...
        return (
            render_template(
                "error.html", error_code=400, error_message="Please select at least one category"
            ),
            400,
        )
    ...
    for category in selected_categories:
        if category not in valid_categories:
            ...
            return (
                render_template(
                    "error.html", error_code=400, error_message="Invalid category selection"
                ),
                400,
            )
    ...
    if num_questions < 1:
        ...
        return (
            render_template(
                "error.html", error_code=400, error_message="Number of questions must be at least 1"
            ),
            400,
        )
    ...
    if num_questions > len(filtered_questions):
        app.logger.warning(
            "Requested questions (%s) exceeds available (%s)",
            num_questions,
    ...
    if time_limit is None or not isinstance(time_limit, int):
        ...
        return (
            render_template(
                "error.html", error_code=400, error_message="Time limit must be a valid number"
            ),
            400,
        )
    ...
    if shuffle_answers_str not in ["true", "false"]:
        ...
        return (
            render_template("error.html", error_code=400, error_message="Invalid shuffle option"),
            400,
        )
...
def show_question() -> Any:
    ...
    if not isinstance(q_index, int) or q_index < 0:
        ...
        return (
            render_template("error.html", error_code=400, error_message="Invalid question index"),
            400,
        )
    ...
    if (
        ...
        or current_question_index >= len(questions)
    ):
        app.logger.error("Invalid question index in selected_indices: %s", current_question_index)
        return (
            render_template(
                "error.html", error_code=400, error_message="Invalid question reference"
            ),
            400,
        )
    ...
    if request.method == "POST":
        ...
        if shuffle_mappings and current_question_index in shuffle_mappings:
            ...
            if not isinstance(correct_answer_index, int):
                ...
                return (
                    render_template(
                        "error.html", error_code=500, error_message="Invalid test configuration"
                    ),
                    500,
                )
        else:
        ...
        if (
            ...
            or correct_answer_index >= num_options
        ):
            app.logger.error(
                "Invalid correct_answer_index: %s for question with %s options",
                correct_answer_index,
                num_options,
            )
            return (
                render_template(
                    "error.html", error_code=500, error_message="Invalid question configuration"
                ),
                500,
            )
